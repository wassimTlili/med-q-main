...
export const PerformancePie: React.FC = () => {
  ...
  const data = useMemo(() => {
    if (!perf) return [];
    return [
      { key: 'correct', label: 'Correct', value: perf.correct, pct: perf.percentCorrect },
      { key: 'wrong', label: 'Incorrect', value: perf.wrong, pct: perf.percentWrong },
      { key: 'partial', label: 'Partial', value: perf.partial, pct: perf.percentPartial }
    ].filter(d => d.value > 0);
  }, [perf]);
  ...
  <CardTitle className="flex items-center gap-2 text-2xl font-semibold bg-gradient-to-r from-blue-600 to-blue-800 dark:from-blue-400 dark:to-blue-600 bg-clip-text text-transparent">
      ...
      <CardContent className="flex-1 flex flex-col md:flex-row gap-10 md:gap-16 items-center md:items-center justify-center md:justify-start py-2">
        ...
        <div className="relative" style={{width:240, height:240}}>
          {loadingPerf ? (
            <div className="absolute inset-0 flex items-center justify-center text-xs text-muted-foreground">Updating...</div>
          ) : (
          ...
          {total > 0 && !loadingPerf && (
            <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="mt-1 text-[10px] tracking-wide text-muted-foreground uppercase">{showPercent ? 'Success Rate' : 'Correct'}</span>
              <span className="mt-0.5 text-[10px] text-muted-foreground">{total} attempts · {timeframe}d</span>
            </div>
          )}
        </div>
        {/* Legend */}
